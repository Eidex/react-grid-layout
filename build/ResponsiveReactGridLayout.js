...
var ResponsiveReactGridLayout = function (_React$Component) {
  ...
  ResponsiveReactGridLayout.prototype.generateInitialState = function generateInitialState() {
    ...
    return {
      ...
      cols: colNo,
      prevProps: _extends({}, this.props)
    };
  };

  ResponsiveReactGridLayout.getDerivedStateFromProps = function getDerivedStateFromProps(nextProps, prevState) {
    // Allow parent to set width or breakpoint directly.
    if (nextProps.width != prevState.prevProps.width || nextProps.breakpoint !== prevState.prevProps.breakpoint || !(0, _lodash2.default)(nextProps.breakpoints, prevState.prevProps.breakpoints) || !(0, _lodash2.default)(nextProps.cols, prevState.prevProps.cols)) {
      /**
       * When the width changes work through breakpoints and reset state with the new width & breakpoint.
       * Width changes are necessary to figure out the widget widths.
       */
      var _breakpoints = nextProps.breakpoints,
          _cols = nextProps.cols,
          _layouts = nextProps.layouts,
      ...
      var lastBreakpoint = prevState.breakpoint;
      var newCols = (0, _responsiveUtils.getColsFromBreakpoint)(newBreakpoint, _cols);
      var setFlag = false;
      var _layout = void 0;
      // Breakpoint change
      if (lastBreakpoint !== newBreakpoint || prevState.prevProps.breakpoints !== _breakpoints || prevState.prevProps.cols !== _cols) {
        // Preserve the current layout if the current breakpoint is not present in the next layouts.
        if (!(lastBreakpoint in _layouts)) _layouts[lastBreakpoint] = (0, _utils.cloneLayout)(prevState.prevProps.layout);

        // Find or generate a new layout.
        _layout = (0, _responsiveUtils.findOrGenerateResponsiveLayout)(_layouts, _breakpoints, newBreakpoint, lastBreakpoint, newCols, compactType);

        // This adds missing items.
        _layout = (0, _utils.synchronizeLayoutWithChildren)(_layout, nextProps.children, newCols, compactType);

        // Store the new layout.
        _layouts[newBreakpoint] = _layout;

        // callbacks
        prevState.prevProps.onLayoutChange(_layout, _layouts);
        prevState.prevProps.onBreakpointChange(newBreakpoint, newCols);
      ...
      prevState.prevProps.onWidthChange(nextProps.width, nextProps.margin, newCols, nextProps.containerPadding);

      // work around so onWidthChange will still be called
      if (setFlag) {
        return {
          breakpoint: newBreakpoint,
          layout: _layout,
          cols: newCols,
          prevProps: _extends({}, nextProps)
        };
      }
    } else if (!(0, _lodash2.default)(nextProps.layouts, prevState.prevProps.layouts)) {
      // Allow parent to set layouts directly.
      var _breakpoint = prevState.breakpoint,
          _cols2 = prevState.cols;

      // Since we're setting an entirely new layout object, we must generate a new responsive layout
      // if one does not exist.

      var newLayout = (0, _responsiveUtils.findOrGenerateResponsiveLayout)(nextProps.layouts, nextProps.breakpoints, _breakpoint, _breakpoint, _cols2, nextProps.compactType);

      return {
        layout: newLayout,
        prevProps: _extends({}, nextProps)
      };
      //this.setState({ layout: newLayout, prevProps: {...nextProps} });
    }
    return null;
  };
